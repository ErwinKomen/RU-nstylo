...
(function ($) {
  $(function () {
    $(document).ready(function () {
      ...
      ru.nstylo.init_event_listeners();
      $('#id_subtype >option').show();
      // Add 'copy' action to inlines
      ru.nstylo.tabinline_add_copy();
    });
  });
})(django.jQuery);
  ...
  ru.nstylo = (function ($, config) {
    ...
    return {
      ...
      init_event_listeners: function () {
        ...
        // $('#id_DCtype').bind('keyup', ru.nstylo.type_change);
        // $('#id_DCtype').bind('change', ru.nstylo.type_change);
        // Bind the change event for text_list.html, 'part-name'
        $("#part-name").bind('change', ru.nstylo.part_change);
        $("#part-name").change(ru.nstylo.part_change);

        // When a text-line is clicked, a waiting symbol should show up
        $("#sentence-list .line-text a").bind('click', ru.nstylo.sent_click);
      },
      ...
      sync_start : function(sSyncType) {
        ...
        ru.nstylo.oSyncTimer = window.setTimeout(function () { ru.nstylo.sync_progress(sSyncType, oJson); }, 3000);
        ...
        $.ajax({
          ...
          "cache": false,
          "success": function (json) { ru.nstylo.sync_stop(sSyncType, json); }
        })(jQuery);
      },
      ...
      sync_progress: function (sSyncType) {
        ...
        $.ajax({
          ...
          "cache": false,
          "success": function (json) {
            ru.nstylo.sync_handle(sSyncType, json);
          }
        })(jQuery);
      },
      ...
      sync_handle: function (sSyncType, json) {
        ...
        switch (json.status) {
          case 'error':
            ...
            $("#sync_details_" + sSyncType).html(ru.nstylo.sync_details(json));
            // Stop the progress calling
            window.clearInterval(ru.nstylo.oSyncTimer);
          ...
          default:
            ...
            $("#sync_details_" + sSyncType).html(ru.nstylo.sync_details(json));
            ru.nstylo.oSyncTimer = window.setTimeout(function (json) { ru.nstylo.sync_progress(sSyncType); }, 1000);
            break;
        }
      },
